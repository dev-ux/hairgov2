...
function addHairstyle(hairstyle) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      port: 443,
      path: '/api/v1/hairstyles/temp-add',
      method: 'POST',
      headers: {
...
async function addAllHairstyles() {
  console.log('🚀 Ajout des 9 hairstyles restants...\n');
  ...
  // Vérification finale
  console.log('\n🔍 Vérification finale des hairstyles...');
  try {
    const checkResponse = await fetch('https://hairgov2.onrender.com/api/v1/hairstyles');
    const data = await checkResponse.json();
    console.log(`📋 Total hairstyles dans la base: ${data.count}`);
    
    if (data.data && data.data.length > 0) {
      console.log('\n📋 Liste complète des hairstyles:');
