...
// Configuration PostgreSQL avec les identifiants depuis les variables d'environnement
...
// Définition des modèles (identiques à ceux de l'application)
...
async function createSalonsInProduction() {
  try {
    console.log('🔍 Connexion à PostgreSQL Production...');
    ...
    console.log(`📊 Salons existants dans PostgreSQL Production: ${existingSalons.length}`);
    
    if (existingSalons.length >= 3) {
      console.log('✅ Les salons existent déjà dans PostgreSQL Production');
    ...
    console.log(`📊 Coiffeurs approuvés dans PostgreSQL Production: ${existingHairdressers.length}`);
    ...
    if (existingHairdressers.length < 3) {
      ...
      for (const hairdresserData of missingHairdressers) {
        const user = await User.create({
          ...hairdresserData,
          password_hash: 'password123' // Sera hashé automatiquement par le hook
        });
    ...
    // Créer les salons
    console.log('🔄 Création des salons dans PostgreSQL Production...');
    ...
    console.log('\n🎉 Succès! Les salons ont été ajoutés à PostgreSQL Production.');
    ...
    console.log('\n📊 Salons dans PostgreSQL Production après création:');
    console.log('================================================');
    ...
    console.error('❌ Erreur lors de la création dans PostgreSQL Production:', error.message);
    if (error.original) {
