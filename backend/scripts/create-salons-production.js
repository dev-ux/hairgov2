...
async function createSalonsInProduction() {
  try {
    console.log('🔍 Vérification de la connexion PostgreSQL en production...');
    
    // Forcer le mode production pour utiliser PostgreSQL
    ...
    const existingSalons = await db.Salon.findAll({
      attributes: ['id', 'name'],
    ...
    // Vérifier les coiffeurs existants
    ...
    // Créer les coiffeurs s'ils n'existent pas
    if (existingHairdressers.length < 3) {
      ...
      // Créer uniquement les coiffeurs manquants
      ...
      for (const hairdresserData of missingHairdressers) {
        ...
        const hairdresser = await db.Hairdresser.create({
          user_id: user.id,
    ...
    console.log('\n🎉 Succès! Les salons ont été ajoutés à PostgreSQL en production.');
    
    // Vérification finale
    const finalSalons = await db.Salon.findAll({
